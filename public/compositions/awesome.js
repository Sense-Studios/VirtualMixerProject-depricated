...
var source4 = new VideoSource(renderer, { src: "/video/ignore/Paprika Dual audio720p.mp4" })
var source5 = new VideoSource(renderer, { src: "/video/ignore/Paprika Dual audio720p.mp4" })
*/
//var source3 = new VideoSource(renderer, { src: "http://nabu-dev.s3.amazonaws.com/uploads/video/53e2a3ad6465761455190000/720p_5000kbps_h264.mp4?r=737324588185" } );
  ...
  var source4 = new VideoSource(renderer, { src: "/video/ignore/le_sacre/lsdp_ballets_russes.mp4" })
  var source5 = new VideoSource(renderer, { src: "/video/ignore/le_sacre/lsdp_nathalie_kasatikina.mp4" })


// create a mixer, mix red and green
var premixer1 = new Mixer( renderer, { source1: source4, source2: source5 });
var premixer2 = new Mixer( renderer, { source1: source1, source2: source2 });

var mixer1 = new Mixer( renderer, { source1: premixer1, source2: premixer2 });
...
audioanalysis1.add( premixer1.pod )
audioanalysis1.add( premixer2.pod )
mixer1.bindBpm( function() { return audioanalysis1.getBpm() / 4 } )
...
setInterval(function() {
  if ( audioanalysis1.render() > 0.99 && !wasSet ) {
    ...
    if (beats == 7) source3.jump()
    if (beats == 8) source4.jump()
    if (beats == 9) source5.jump()

    //if (beats == 6) filemanager2.changez()
    if (beats%4 == 0 && dice < 0.1 ) source1.jump()
    if (beats%4 == 0 && dice < 0.1 ) source2.jump()
    if (beats%4 == 0 && dice < 0.1 ) source3.jump()
    if (beats%4 == 0 && dice < 0.1 ) source4.jump()
    if (beats%4 == 0 && dice < 0.1 ) source5.jump()
